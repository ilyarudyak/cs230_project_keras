...
class Unet:
    ...
    def contracting_block(self, filters, input_layer, name, maxpooling=True):
        if maxpooling:
            ...
            maxpool = input_layer
        conv = layers.Conv2D(filters=filters, kernel_size=3, activation='relu', padding=self.padding,
                             name=name + "_conv_1")(maxpool)
        conv = layers.Conv2D(filters=filters, kernel_size=3, activation='relu', padding=self.padding,
    ...
    def expanding_block(self, filters, input_layer, concat_layer, name):
        ...
        conv = layers.Conv2D(filters=filters, kernel_size=2, activation='relu', padding='same', name=name + "_conv_0")(
            ups)
        # compute cropping
